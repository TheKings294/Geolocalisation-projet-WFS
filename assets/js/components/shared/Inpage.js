...
export const getInpage = (total, curentPage, situation, component, sens, action) => {
    ...
    handleInpageClick(curentPage, situation)
}

export const handleInpageClick = (curentPage, situation) => {
    ...
    previousLink.addEventListener('click', async () => {
        if(curentPage > 1) {
            curentPage--
            await refreshPage(curentPage, situation, previousLink.getAttribute('data-component'), previousLink.getAttribute('data-sens'), previousLink.getAttribute('data-action'))
        }
    })

    nextLink.addEventListener('click', async () => {
        curentPage++
        await refreshPage(curentPage, situation, nextLink.getAttribute('data-component'), nextLink.getAttribute('data-sens'), nextLink.getAttribute('data-action'))
    })

    nbPageLink.forEach(btn => {
        btn.addEventListener('click', async (e) => {
            await refreshPage(e.target.getAttribute("data-page"), situation, btn.getAttribute('data-component'), btn.getAttribute('data-sens'), btn.getAttribute('data-action'))
        })
    })
}
