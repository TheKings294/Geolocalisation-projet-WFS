...
export const getInpage = (total, curentPage, situation, component, sens, action, editLink) => {
    ...
    inpageElement.innerHTML += `<li class="page-item">
                                    <a class="page-link" href="#" id="prev-link" 
    ...
    for (let i = 0; i < nbPage; i++) {
        inpageElement.innerHTML += `<li class="page-item">
                                        <a class="page-link nb-page-link" href="#" 
                                        data-page="${i+1}" 
                                        data-component="${component}" 
                                        data-sens="${sens}" 
                                        data-action="${action}">
    ...
    inpageElement.innerHTML += `<li class="page-item">
                                    <a class="page-link" href="#" id="next-link" 
                                    data-component="${component}" 
                                    data-sens="${sens}" 
                                    data-action="${action}">
                                    Next</a>
...
export const handleInpageClick = (curentPage, situation, editLink) => {
    ...
    previousLink.addEventListener('click', async () => {
        if(curentPage > 1) {
            ...
            await refreshPage(curentPage,
                situation,
                previousLink.getAttribute('data-component'),
                previousLink.getAttribute('data-sens'),
                previousLink.getAttribute('data-action'),
                editLink)
        }
    })
    ...
    nbPageLink.forEach(btn => {
        btn.addEventListener('click', async (e) => {
            await refreshPage(e.target.getAttribute("data-page"),
                situation,
                btn.getAttribute('data-component'),
                btn.getAttribute('data-sens'),
